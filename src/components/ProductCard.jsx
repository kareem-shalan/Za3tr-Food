...
const ProductCard = () => {
  ...
  const { FristSection } = useContext(ContextMeals);

  useEffect(() => {
    const fetchMeals = async () => {
      setIsLoading(true);
      try {
        const response = await FristSection();
        if (response?.meals) {
          // Limit to 8 meals and ensure they're unique
          const uniqueMeals = response.meals
            ...
            <div className="p-4">
              <div className="flex justify-between items-center mb-2">
                <span className="px-2 py-1 bg-primary/10 text-primary rounded-full text-xs flex items-center gap-1">
                  <span className="text-base">{getCategoryEmoji(meal.strCategory)}</span>
